...
function html_section($posts, $parameter) {
  ...
  if( $posts ) {
      ...
      $taxonomies = get_the_terms( $post->ID, 'post_tag' );
      ...
      $html .= '
      <article class="w-grid-item post-'.$post->ID.' post type-post status-publish format-standard has-post-thumbnail hentry category-events tag-'.get_query_var( 'eventcity' ).'" data-id="'.$post->ID.'">
      		<div class="w-grid-item-h">
                  ...
                  foreach ($slugs as $slug ) {
                    $html .= ' <a class="w-btn us-btn-style_3" href="'.get_site_url().'/'.$tax_slug.'/category/'.$slug->slug.'"><span class="w-btn-label">'.$slug->name.'</span></a>';
                  }
 ...
 $html .= '
  ...
  </div></div></div><div class="wpb_column vc_column_container us_custom_72fc42ef type_sticky"><div class="vc_column-inner" style="top:"><div class="g-cols wpb_row us_custom_6217dcc9 via_grid cols_1 laptops-cols_inherit tablets-cols_inherit mobiles-cols_1 valign_top type_default"><div class="wpb_column vc_column_container"><div class="vc_column-inner"><div class="w-image align_none">'.do_shortcode('[adsense_vertical_ad]').'<div class="w-image-h"></div></div></div></div></div></div></div></div></div>
  </section>';
